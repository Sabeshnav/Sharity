...
class SuperView(views.MethodView):
    ...
    def update(self, obj_id, data, resource = None, projection = None,user_id = None):
        resource = resource if resource else self.resource
        mask = self.projection_helper(projection) if projection else self.mask
        ...
        if user_id:
            ...
            if not self.db["user"].find_one({"_id":user_id}):
                raise BussinessException("error",400,"No such user exists")
        if not obj_id:
    ...
    def remove(self,user_id, obj_id, resource = None, projection = None):
        resource = resource if resource else self.resource
        mask = self.projection_helper(projection) if projection else self.mask
        obj_id = convertToObjectId(obj_id)
        if user_id:
    ...
    def retrieve(self, obj_id, resource = None, projection = None, user_id = None):
        resource = resource if resource else self.resource
        mask = self.projection_helper(projection) if projection else self.mask
        obj_id = convertToObjectId(obj_id)
        if user_id:
